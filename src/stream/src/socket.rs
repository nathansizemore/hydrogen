...
impl Read for Socket {
    fn read(&mut self, buf: &mut [u8]) -> Result<usize, Error> {
        if buf.len() < 1 {
            return Err(Error::new(ErrorKind::Other, ReadError::InvalidBuffer))
        }
        ...
        if result < 0 {
            return Err(Error::new(ErrorKind::Other, ReadError::from_errno(result as i32)))
        }
...
impl Write for Socket {
    fn write(&mut self, buf: &[u8]) -> Result<usize, Error> {
        ...
        if result < 0 {
            return Err(Error::new(ErrorKind::Other, WriteError::from_errno(result as i32)))
        }
